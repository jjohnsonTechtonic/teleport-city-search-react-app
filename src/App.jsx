...
const App = () => {
  ...
  const setCityDetails = (url) => {
    //api call 1
    getCityDetailsByLink(url).then(res => res.json())
    .then(data => {getCityDetails(data)}).catch(err => console.error(err));
  }

  useEffect(() => {
    if (cityDetails){
      getImageByCity(cityDetails.name).then(res => {
        if (res.ok){
          return res.json();
        }
      }).then(data => {
  ...
  {cityDetails?<CityDetails details={cityDetails} imageUrl={imageUrl}/>:<></>}
  </main></>;
};
