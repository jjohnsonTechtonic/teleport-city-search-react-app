...
const CityScores = ({cityName}) => {
    ...
    useEffect(() => {
        getScoresByCity(cityName).then(res => {
                if (res.ok){
                    return res.json();
                }
            }).then(data => {
    ...
    return <div className="details__scores">
            ...
            <>
                ...
                <ul>
                    {scoreDetails.categories.map(({name,score_out_of_10}) => {
                        return <li key={`${name}-${score_out_of_10}`}>{name}: <span>{score_out_of_10.toFixed(1)}</span>/10</li>
                    })}
                </ul>
            </>
